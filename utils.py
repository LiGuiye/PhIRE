...
def conv_layer_2d(x, filter_shape, stride, trainable=True):
    ...
    x = tf.nn.bias_add(
        tf.nn.conv2d(input=x, filter=W, strides=[1, stride, stride, 1], padding='SAME'),
        b,
    )
...
def plot_SR_data(idx, LR, SR, path):
    ...
    for i in range(LR.shape[0]):
        ...
        plt.subplot(221)
        plt.imshow(
            LR[i, :, :, 0], vmin=vmin0, vmax=vmax0, cmap='viridis', origin='lower'
        )
        ...
        plt.subplot(223)
        plt.imshow(
            LR[i, :, :, 1], vmin=vmin1, vmax=vmax1, cmap='viridis', origin='lower'
        )
        ...
        plt.subplot(222)
        plt.imshow(
            SR[i, :, :, 0], vmin=vmin0, vmax=vmax0, cmap='viridis', origin='lower'
        )
        ...
        plt.subplot(224)
        plt.imshow(
            SR[i, :, :, 1], vmin=vmin1, vmax=vmax1, cmap='viridis', origin='lower'
        )
        ...
        plt.savefig(
            path + '/img{0:05d}.png'.format(idx[i]), dpi=200, bbox_inches='tight'
        )
        plt.close()
...
def downscale_image(x, K):
    ...
    weight = tf.constant(
        1.0 / K**2, shape=[K, K, x.shape[3], x.shape[3]], dtype=tf.float64
    )
